...
extractDataFromSimulation <- function(sim,refSim) {
  pdead_ifsev_nv <- c(0,0,0.07,0.07,0.07,0.10,0.35,0.40,0.60)
  pdead_ifsev_v <- c(0,0,0,0,0,0.08,0.23,0.30,0.45)
  L<-parms$Tmax
  detected<-sum(sim$detected[1:L,])
  severe<-sum(sim$sevp[1:L,])
  mortality<-sum(colSums(sim$sevp_nv[1:L,])*pdead_ifsev_nv+colSums(sim$sevp_v[1:L,]+sim$sevp_b[1:L,])*pdead_ifsev_v)
